...
const toObjectUrl = (base64, mimeType = "image/jpeg") => {
  try {
...
function clearBlobUrl(filename) {
  const current = blobCache[filename];
...
async function evictIfNeeded(db) {
  try {
    ...
    for (const entry of expired) {
      ...
      clearBlobUrl(entry.key.replace(PREFIX, ""));
    }
...
const ImageCache = {
  ...
  // On a new deploy we keep existing images instead of forcing a full refetch.
  ...
  getOrCreateObjectUrl(filename, base64, mimeType = "image/jpeg") {
    if (blobCache[filename]) {
      return blobCache[filename];
    }

    const objectUrl = toObjectUrl(base64, mimeType);
    blobCache[filename] = objectUrl;
    return objectUrl;
  },
  ...
  async clearAll() {
    try {
      ...
      Object.keys(blobCache).forEach((key) => clearBlobUrl(key));
